...
- `services/estimate_service.py` — **сопоставление позиций сметы с каталогом,
  ...
  переиспользует `get_llm_mapping`); без LLM — топ ретрива.
...
### ПОТОМ
Запись в `estimates`/`estimate_items`, эндпоинты `POST /api/estimates/upload` (фон) +
`GET /api/estimates/{id}` + выбор товара/поставщика + экспорт; UI-раздел смет;
фильтр поставщиков.
