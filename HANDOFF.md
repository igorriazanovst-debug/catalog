...
- `src/pages/EstimatesPage.tsx` — список смет (сумма, подбор, удаление).
- `src/pages/EstimateUploadPage.tsx` — загрузка сметы (xlsx) + выбор LLM-провайдера/
  декомпозиции/цены, фоновый подбор с прогрессом → переход в детальную.
- `src/pages/EstimateDetailPage.tsx` — позиции сметы с подобранным товаром/ценой,
  группировка наборов (`group_name`), ручной выбор товара (candidates→choose),
  экспорт xlsx. `api.ts` дополнен типами/клиентом смет (см. функции `*Estimate*`).
...
- Фильтр поставщиков (пока подбор из ВСЕХ; UI-загрузка это уже предусматривает в
  будущем — добавить выбор поставщиков в форму и параметр в `EstimateMatcher`).
- Предметные промахи матча товаров (рус/англ таблицы во вложении сметы-2) — это
  нюанс маппинга ТОВАРОВ (`mapping_service`/промпт судьи), не смет.
- НДС в детальной смете UI (сейчас показываем итог без НДС; НДС есть в xlsx-экспорте).
