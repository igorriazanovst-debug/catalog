...
```
### СЛЕДУЮЩИЙ ШАГ (приоритет пользователя): заполнить коды КТРУ/ОКПД2
Чтобы включить точный код-матч, нужны коды в каталоге:
  * `industry_standards.ktru_code/okpd2_code` — источник `838.xlsx` (если там есть
    колонка кода). Структуру НЕ знаем → сперва `scripts/inspect_columns.py 838.xlsx`.
  * `products.ktru_code/okpd2_code` — источник прайс поставщика (если есть колонка).
    Проверить: `scripts/inspect_columns.py <прайс.csv>`; если код есть — дочитывать
    его в `product_service.import_products_from_csv` и/или разовый backfill-скрипт.
Порядок: (1) пользователь гоняет `inspect_columns.py` на 838.xlsx и прайсе, шлёт
вывод; (2) пишем `scripts/backfill_codes.py` (миграция кодов в БД) под реальные
колонки; (3) перепроверяем `match_estimate.py` — теперь метод `ktru/okpd2`.

Потом: запись в `estimates`/`estimate_items`, эндпоинты `POST /api/estimates/upload`
(фон) и UI; подключение LLM-судьи к текстовому фоллбэку (`llm_mapping_service`);
фильтр поставщиков.
